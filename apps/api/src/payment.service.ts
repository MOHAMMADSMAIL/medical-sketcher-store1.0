...
export class PaymentService {
  ...
  private async applyResult(paymentId: string, result: { status: 'SUCCEEDED' | 'FAILED'; transactionId?: string; resultCode: string; resultDescription?: string }) {
    ...
    if (result.status === 'SUCCEEDED') {
      const order = await this.prisma.order.findUniqueOrThrow({ where: { id: payment.orderId }, include: { user: true, items: { include: { product: true } } } });
      await this.email.send({ to: order.user.email, template: 'payment-confirmation', variables: { orderId: order.id } });
      await Promise.all(order.items.map(item => this.email.send({ to: order.user.email, template: 'download-ready', variables: { productTitle: item.product.title } })));
    }
    return payment;
  }
}
